	    ...
	    success:function(data) {
	    	...
	    	if(data.msg != "fail"){
	    		if("has no code" == data.goldCode){
	    			...
	    			executeInfo.html(executeInfo.html()+"当前金额阈值编号为"+data.goldCode+"<br />");
	    		}
	    		if(data.toMuchGold){
	    			executeInfo.html(executeInfo.html()+"表格总金额超过当前所设阈值！！！");
	    		}
	    		
	    		if(data.toMuchGold){
	    			showMessage.html(showMessage.html()+"表格总金额超过当前所设阈值！！！"+"<br />");
	    		}
	    		$.each(data.msg, function(a,b){
	    			showMessage.html(showMessage.html()+b+"<br />");
	    		});
	    		//判断能否执行
		    	if(data.canBalance){
		    		batchView.show();
		    		executeInfo.html(executeInfo.html()+"恭喜，表格可以批量入金");
		    		executeGload.removeAttr("disabled");
		    		return;
		    	}
	    	}else{
	    		executeInfo.html("表格有重复用户！！！<br />");
	    		showMessage.html(showMessage.html()+"表格有重复用户！！！"+"<br />");
	    	}
...
function showResult(){
	...
	$.ajax({
	    ...
	    success:function(data) {
	    	...
	    	balanceResult.html(balanceResult.html()+"一共处理"+data.total+"条数据，成功执行"+data.successCount+"条数据,执行失败"+data.errorCount+"条数据 <br />");
