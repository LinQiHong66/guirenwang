...
public class BatchAddBalance implements ExcelBatch{
    ...
    public synchronized boolean addBalance(String phone, int money) throws Exception{
    	...
    	String sql1 = "update t_inesv_user_balance set total_price=unable_coin+enable_coin where coin_type=60 and user_no = "+userNo;
    	...
    	for(int k = 0; k < money/1000; k ++) {
    		String sql2 = "insert into t_crowdfunding_details (user_id,ico_id,ico_user_number,ico_user_sumprice,date) VALUES ("
    				+ userNo + ", 201708130000000002, 333,1000, now())";
    		c += queryRunner.update(sql2);
    	}
    	if(a >= 1 && b >= 1 && c >= money/1000) {
    		return true;
    	}
    	return false;
    }
    private int getUserNo(String phone) throws SQLException {
		int userNo = 0;
		List<Integer> nos = queryRunner.query("select user_no as userNo from t_inesv_user where phone='"+phone+"'", new ColumnListHandler<Integer>("userNo"));
	...
	public boolean isReg(String phone) throws SQLException {
		List<InesvUserDto> dtos = queryRunner.query("select * from t_inesv_user where username="+phone, new BeanListHandler<>(InesvUserDto.class));
	...
	public StaticParamsDto getParam(String key) throws SQLException {
		...
		List<StaticParamsDto> dtos = queryRunner.query(sql, new BeanListHandler<StaticParamsDto>(StaticParamsDto.class));
		if(dtos != null && dtos.size() > 0) {
			return dtos.get(0);
		}
		return null;
