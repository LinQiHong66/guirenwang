...
public class TradeAutualPersistence {
	...
	public EntrustDto buy_sell_TradeAutual(String buyTradeType,EntrustDto buyEntrust,EntrustDto sellEntrust,BigDecimal buy_poundatge,BigDecimal sell_poundatge,BigDecimal tradeNum,BigDecimal buyPrice,BigDecimal sellPrice) throws Exception{
		...
		if(buyEntrust.getConvert_coin() == 0) {
			...
			double price = buyEntrust.getEntrust_price().doubleValue()/buyEntrust.getConvert_price().doubleValue()*tradeNum.doubleValue();
			...
			buyRmb.setUnable_coin(new BigDecimal(0));
		}
	...
	public void TradeSql(String entrustType,String sqlType,EntrustDto buy_sell_EntrustDto,List<EntrustDto> buy_sell_entrustList,BigDecimal buy_poundatge,BigDecimal sell_poundatge) throws Exception{
		...
		if(buy_sell_EntrustDto.getEntrust_type()==0){
			getUnSellList=queryEntrustByEntrustPriceEntrustCoinAndEntrustTypeAndState(buy_sell_EntrustDto);
			//卖方无数据不交易
			if(getUnSellList!=null && getUnSellList.size()>0){
				...
				// 先交易
				buy_sell_EntrustDto = buy_sell_TradeAutual(entrustType,buy_sell_EntrustDto,sellEntrust,
		...
		if(buy_sell_EntrustDto.getEntrust_type()==1){
			...
			if(getUnSellList!=null && getUnSellList.size()>0){
				...
				// 先交易
				buy_sell_EntrustDto = buy_sell_TradeAutual(entrustType,buyEntrust,buy_sell_EntrustDto,
						buy_poundatge, sell_poundatge,
	 ...
	 * @throws SQLException 
	 */
	public List<EntrustDto> queryEntrustByEntrustPriceEntrustCoinAndEntrustTypeAndState(EntrustDto buy_sell_EntrustDto) throws Exception{
		String sql = null;
		List<EntrustDto> list = null;
		if(buy_sell_EntrustDto.getEntrust_type() == 0) {
			sql="SELECT * FROM t_inesv_entrust WHERE entrust_price=? AND entrust_coin =? AND entrust_type=? AND state=? AND convert_coin = ? AND convert_price = ? AND entrust_num!=deal_num ORDER BY date ASC FOR UPDATE";
			Object params1[] = {buy_sell_EntrustDto.getEntrust_price(),buy_sell_EntrustDto.getEntrust_coin(),1,0,buy_sell_EntrustDto.getConvert_coin(),buy_sell_EntrustDto.getConvert_price()};
			list = queryRunner.query(sql,new BeanListHandler<EntrustDto>(EntrustDto.class),params1);
		}else if(buy_sell_EntrustDto.getEntrust_type() == 1) {
			sql="SELECT * FROM t_inesv_entrust WHERE entrust_price=? AND entrust_coin =? AND entrust_type=? AND state=? AND convert_coin = ? AND convert_price = ? AND entrust_num!=deal_num ORDER BY date ASC FOR UPDATE";
			Object params2[] = {buy_sell_EntrustDto.getEntrust_price(),buy_sell_EntrustDto.getEntrust_coin(),0,0,buy_sell_EntrustDto.getConvert_coin(),buy_sell_EntrustDto.getConvert_price()};
			list = queryRunner.query(sql,new BeanListHandler<EntrustDto>(EntrustDto.class),params2);
		}
		return list;
	}
